...
required_libraries <- c("ggplot2", "dplyr", "skimr", "shiny", "DT", "here", "rsconnect", 
                        "tidymodels", "tidyverse", "yardstick", "MASS", "broom")
...
null_preds_df <- tibble(
  Y = observed_values,
  Predicted = mean_y, # mean_y calculated earlier for the null model
  Model = "NULL"
)
...
ggplot(all_preds_df, aes(x = Y, y = Predicted, color = Model)) +
  geom_point() +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "black") +
  scale_color_manual(values = c("DOSE" = "purple", "ALL" = "orange", "NULL" = "blue")) +
...
```{r}
all_residuals <- all_preds_df %>% 
  filter(Model == "Full Model") %>% 
  mutate(Residuals = Predicted - observed_values)

# Plotting Predicted vs Residuals for Full Model

ggplot(all_residuals, aes(x = Predicted, y = Residuals)) +
  geom_point() +
  geom_abline(yintercept = 0, linetype = "solid", color = "red") +
  labs(x = "Predicted Values", y = "Residuals", title = "Predicted vs Residuals for Full Model")+
...
```{r}
# Setting the same seed used in the beginning of this exercise
set.seed(rngseed2)

# Creating 100 bootstrap sammples from training data
bootstrap_training <- bootstraps(mav10_train_75pct, times = 100) 

# Creating a function to fit the model to each bootstrap sample
bootstrap_pred <- bootstrap_training$splits %>% 
  map(.f = ~{
  bootstrap_all_var <- analysis(.x)
    fit_model <- fit(model_spec_all_new, data = bootstrap_all_var)

# Now to make predictions on the original training data 
predict(fit_model, new_data = mav10_train_75pct)$.pred 

}) # took hours to realize that this entire thing is a function wow...

#Converting the list of prediction vectors into a matrix
bootstrap_predictions_matrix <- do.call(rbind, bootstrap_pred)

# Calculating mean predictions for each sample
mean_predicitions <- rowMeans(bootstrap_predictions_matrix)

# Calculating median and CI from webpage example
median_CI89 <- bootstrap_predictions_matrix %>% 
  apply(1, quantile, c(0.055, 0.5, 0.945)) %>% 
  t()

# Creating dataframe from bootstrap_predictions_matrix
median_CI89_df <- as.data.frame(median_CI89)
names(median_CI89_df) <- c("LowerBound", "Median", "UpperBound")

# Adding observed values and original predictions to mav10_train_75pct
median_CI89_df <- mutate(median_CI89_df, Observed = mav10_train_75pct)

ggplot(median_CI89_df, aes(x = Observed)) +
  geom_point(aes(y = preds_all_df$.pred, color = "Original Pred")) +
  geom_point(aes(y= Median, color = "Median Predictions")) +
  geom_errorbar(aes(ymin = LowerBound, ymax = UpperBound, y = Median, color = "Confidence Intervals")) +
  geom_abline(yintercept = 0, slope = 1, linetype = "solid", color = "gold") +
  labs(x = "Observed Values", y = "Predictions", title = "Predicted vs Observed Values")+
  scale_x_continuous(limits = c(-3000, 3000)) +
  scale_y_continuous(limits = c(-3000, 3000)) +
  theme_minimal()
